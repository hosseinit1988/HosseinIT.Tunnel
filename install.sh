...
Lena_menu() {
    ...
    echo "+-----------------------------------------------------------------------------+"
    echo "|                            VXLAN TUNNEL SETUP                              |"
    echo "+-----------------------------------------------------------------------------+"
    echo -e "| Telegram Channel : ${MAGENTA}@Hossein.IT ${NC}| Version : ${GREEN} 1.0.2 Beta ${NC} "
    echo "+-----------------------------------------------------------------------------+"
...
if [[ "$role_choice" == "1" ]]; then
    ...
    read -p "Should port forwarding be done automatically via HAProxy? [1-yes, 2-no]: " haproxy_choice
    if [[ "$haproxy_choice" == "1" ]]; then install_haproxy_and_configure; fi
...
ip link show $VXLAN_IF >/dev/null 2>&1 && ip link del $VXLAN_IF
ip link add $VXLAN_IF type vxlan id $VNI local $(ip route get 1.1.1.1 | awk '{print $7}') remote $REMOTE_IP dev $INTERFACE dstport $DSTPORT nolearning
